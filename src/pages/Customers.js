...
const EnhancedTableToolbar = (props) => {
  ...
  const {numSelected, selectedItem , getDataCustomer} = props;
  const [showadd, setShowAdd] = useState(false)
  const [showedit, setShowEdit] = useState(false)
  const [info, setInfo] = useState(initialState)
  const {username, customer_name, customer_address, customer_phone} = info
  ...
  const handleSubmitAddData = async (event) =>
  {
    ...
    const res = await Axios.post(`http://localhost:3002/customer/check-username/${info.username}`)
    if (res.data.check == "false")
    {
      Axios.post(`http://localhost:3002/customer/add-customer/`, info)
  ...
  const handleSubmitEditData = async (event) =>
  {
    ...
    //const res = Axios.post(`http://localhost:3002/account/check-username`, info.username)
      ...
      Axios.patch(`http://localhost:3002/customer/update-customer/`, info)
